...
# v0 - Initial script
# v1 - Update for most current version.
# ----------------------------------------------------------------------------- 
...
dir = 'D:/projects/CDPOP/DGS2018_Project2_HybridSampling/runs/initial_Lindsey/secondRun/'
plottitle = 'Complete Barrier'
...
for ibatch in range(batchno):
	...
	for imc in range(mcno):
		...
		for iout in range(gen):
			...
			totM[ibatch][imc].append(int(values[1+iout][6].split('|')[0]))
			breedF[ibatch][imc].append(int(values[1+iout][8].split('|')[0]))
			breedM[ibatch][imc].append(int(values[1+iout][10].split('|')[0]))
			births[ibatch][imc].append(int(values[1+iout][15].split('|')[0]))
...
# Fst
Fst_m = 1. - (Ho_m[0,:,0]/He_m[0,:,0])
Fst_l = 1. - (Ho_l[0,:,0]/He_l[0,:,0])
Fst_r = 1. - (Ho_r[0,:,0]/He_r[0,:,0])
...
for i in xrange(len(N_age_m[0][0])):
...
for tick in ax.yaxis.get_major_ticks():
	tick.label1.set_fontsize(fontsize)
savefig(dir+savename+'GlobalF.png',dpi=savedpi)
